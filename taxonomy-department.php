...
// Get the current department term
$department = get_queried_object();

// 1. Fetch the Cover Image 
...
<main class="department-archive-main bg-white pb-24">
    ...
    <?php if ($cover_id): ?>
        <section class="relative w-full max-w-7xl mx-auto px-6 mt-6 mb-10">
            <!-- Flatter aspect ratio (h-64 to h-80 style proportions) so it's not too tall -->
            <div
                ...
                <!-- Gradient Overlay (Dark at the bottom fading up) -->
    ...
    if (have_rows('department_sections', $department)):
        ...
        while (have_rows('department_sections', $department)):
            ...
            // Dynamically load the template part based on the layout name
            get_template_part('template-parts/sections/' . $layout, null, ['department' => $department]);

        endwhile;

    else:
        // Fallback if no sections are added yet
        ?>
